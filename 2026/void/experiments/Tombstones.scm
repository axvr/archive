...
;;
;; Would be good to attach metadata to nil
;;   but unable in Clojure.
;; wrap nil in a void object?
;;   then not compatible elsewhere.
;;
;; Voidable as a protocol?
;;   .voidable to create a voidable
;;     version.
;;   .nilVoidable to automatically void
;;     nils.
;;
;; Vector example too.
;;
;; Can I use a reader tag/macro to swap
;;   out a data structure literal?
;;   alternatively there is record syntax?

#void-if-nil
;; no point in creating many different
;;   variants of this.  if it is a value
;;   it is more flexible than a reader tag
;;   pretending to be a function.

;; blog post structure:
;; - problem statement with example
;;   (Clojure and Elixir?)
;;   even worse when the languages don't
;;   include an assoc-some, update-some
;;   or maybe_update, etc.
...
;;     - values or tags?
;; - potential problems
;;   - if it exists needs to be default
;;     in data structures and language
;;     and have similar nil properties.
;;     (needs to be a superset of nil)
;;     which is impossible in Java and
;;     most lisps/languages.
;;   - what happens when you use void
;;     elsewhere?  should void usage be
;;     contrained
;;   - if void is not a singleton object
;;     it could be accidentally or
;;     maliciously used
;; - later
...
(deftype TombstoneMap [m]
  clojure.lang.IMap
  (assoc [this key val]
    (if (identical? val void)
      (dissoc m key)
      ))
  (update ))

;; (reject nil?) or (strip-nils) don't
;; work when you need to include some nil
;; values.  but even if they do work for
;; your use, they don't make the intent
;; clear on which values are supposed to
;; be optional and which are not.
;;
;; Performance too.  why construct the
;; data structure to immediately remove
