...
sub central_ctrl {
	...
	sub wipe {
		...
		my (@partitions) = (@_);
		foreach (@partitions) {
			print $fh STDERR "Unmounting partition $_\n";
			system("umount", "$_");
			print $fh STDERR "$_ was unmounted\n";
		}
		...
		sub wipe {
			print $fh STDERR "This will take a while, it may look like ALIS has frozen\n"
			foreach (@partitions) {
				print $fh STDERR "Wiping $_";
				#dd if=/dev/urandom of=/dev/sdX bs=1M
				system("dd", "if=/dev/$operation", "of=$_", "bs=1M");
				print $fh STDERR "$_ was wiped";
			}
		}
	...
	sub auto {
		print $fh STDERR "auto() has not yet been written\n"
	}
